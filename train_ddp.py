...
def main(gpu, opt):
    ...
    for epoch in tqdm(range(1)):
        ...
        for i_train_batch, train_batch in enumerate(trainloader):
            ...
            # pix_model.set_input_with_face_mask(images, head_bboxs, uv_imgs, Ts, G_tsf, masks)
            pix_model.set_input_with_face(masks.expand(-1,3,-1,-1)*images, head_bboxs, uv_imgs, Ts, G_tsf)
            pix_model.optimize_parameters()

            if dist.get_rank() == 0:
                if not (i_train_batch+epoch*data_len) % 50:
                ...
                if not (i_train_batch+epoch*data_len) % 200 and i_train_batch != 0:
