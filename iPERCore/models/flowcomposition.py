...
class FlowComposition(torch.nn.Module):
    ...
    def mk_data(self, images, smpl, mask):
        ...
        src_smpl = smpl
        ref_smpl = smpl[:,1:]
        bs,ns,_,h,w = images.shape
        nt = 1

        self.make_uv_setup(bs, ns, self._opt.time_step, src_img.device)
        src_links_ids = None
        ref_links_ids = None
        src_info = self.smpl.get_details(src_smpl.view(bs * ns, -1), 0, links_ids=src_links_ids)
        ref_info = self.smpl.get_details(ref_smpl.view(bs * nt, -1), 0, links_ids=ref_links_ids)

        if mask is not None:
            src_info["masks"] = mask.view((bs * ns, 1, h, w))

        if mask is not None:
            ref_info["masks"] = mask[:,1].view((bs * nt, 1, h, w))
        ...
        uv_fim, uv_wim = self.render.render_uv_fim_wim(bs * nt)
        Tsrc2uv = self.render.cal_bc_transform(ref_info["only_vis_f2pts"], uv_fim, uv_wim)
        ref_uv = F.grid_sample(ref_img.view(bs * nt, 3, h, w), Tsrc2uv).view(bs, nt, -1, h, w) 

        uv_img, input_G_bg, input_G_src = self.process_source(src_img, src_info, primary_ids=None)
