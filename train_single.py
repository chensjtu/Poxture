...
from models.pix2pix_model import Pix2PixModel
    ...
    for epoch in tqdm(range(opt.n_epochs+opt.n_epochs_decay)):
        ...
        for i_train_batch, train_batch in enumerate(trainloader):
            ...
            # pix model: time: 0.8s
            pix_model.set_input_with_face(masks.expand(-1,3,-1,-1)*images[:,1], head_bboxs, uv_imgs[:, 0], Ts)
            ...
            if dist.get_rank() == 0:
                ...
                if not (i_train_batch+epoch*data_len) % opt.Train.display_freq_s:
                    ...
                    visualizer.vis_named_img('epoch: {},iter: {}, complete_feat'.format(epoch,i_train_batch),pix_model.get_current_visuals()['complete_feat'][0:1,:3])
                ...
                if not (i_train_batch+epoch*data_len) % opt.Train.save_latest_freq_s and epoch != 0:
