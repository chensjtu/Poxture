...
from iPERCore.data.processed_video_dataset import ProcessedVideoDataset_for_cross_imitation
...
def main(opt):
    ...
    noval_smpls = create_T_pose_novel_view_smpl(length=180)  
    ...
    train_dataset = ProcessedVideoDataset_for_cross_imitation(opt, is_for_train=False)
    ...
    with torch.no_grad():
        for index, train_batch in enumerate(tqdm(testloader)):
            ...
            smpls: [b,2,85]
            '''
            noval_smpls = torch.from_numpy(noval_smpls).to(device)
            ...
            noval_smpls[:, 6:] = smpls[0,0,6:]
            noval_smpls[:, 0:3] = smpls[0,0,0:3]
            for i in range(noval_smpls.shape[0]):
                smpls[:,1:2] = noval_smpls[i]
                ...
                img = save_cv2_img(final_image, '{}/{}/{:05d}.jpg'.format(opt.results_path, opt.name, i),normalize=True)
                # if index > 199:
                #     break
                video.write(img)
        #     # if index > 119:
