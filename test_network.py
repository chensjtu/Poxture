...
from iPERCore.data.processed_video_dataset import ProcessedVideoDataset_for_test
...
def main(opt):
    ...
    train_dataset = ProcessedVideoDataset_for_test(opt, is_for_train=False)
    ...
    with torch.no_grad():
        for index, train_batch in enumerate(tqdm(testloader)):
            ...
            fake = pix_model.test(uv_imgs, Ts, G_tsf)
            '''
            # there are two different ways to process the results. Here, we use while as the bg color.
            # fake_with_bg = torch.where(masks.expand(-1,3,-1,-1)<0.5, bg, fake)
            # fake_in_one = torch.sum(torch.abs(fake), dim=1)
            # fake_mask = torch.sign(torch.clamp(fake_in_one-(1e-1), 0, 1))
            # fake_with_bg = bg*((fake_mask*-1)+1)+fake*fake_mask
            # final_image = torch.cat((images, fake_with_bg), 3)[0].cpu().numpy()
            '''
            visualizer.vis_named_img("deat", standard_vis(pix_model.complete_feat[:,:3]))
            new_img = torch.where(masks.expand(-1,3,-1,-1) > 0.5, images, torch.tensor(1.).to(device))
            # new_img = torch.where(masks.expand(-1,3,-1,-1) > 0.5, images, torch.tensor(0.).to(device))
