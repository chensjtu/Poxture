...
def main(opt):
    ...
    for epoch in tqdm(range(30)):
        for i_train_batch, train_batch in enumerate(trainloader):
            ...
            # premodel: 0.3s
            A_feat, B_feat, loss1, loss2,  = pre_model(uv_imgs[:, 0],uv_imgs[:, 1],ref_uvs[:, 1]) # complete_feat:B 16 512 512
            ...
            if not i_train_batch % 100:
                ...
                visualizer.vis_named_img('epoch: {},iter: {}, comple_A'.format(epoch, i_train_batch), A_feat[0:1])
                visualizer.vis_named_img('epoch: {},iter: {}, comple_B'.format(epoch, i_train_batch), B_feat[0:1])
