# this py is cloned by pix2pix model
import torch
...
class pix2pixModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netNU = NestedUNet(in_ch=3, out_ch=3)
        self.netNU.cuda()
    ...
    def set_input_with_face(self, target, face_bbox, uv_A, Ts, uv_B = 0,index = 0):
        """Unpack input data from the dataloader and perform necessary pre-processing steps.
    ...
    def backward_G(self):
        ...
        # Third, cal the percetual loss and face loss
        self.loss_G_percetual = self.Percetual_loss(self.fake_B, self.real_B)*5
        self.loss_G_face = self.Face_loss(self.fake_B, self.real_B, bbox1=self.face_bbox, bbox2=self.face_bbox)*50
        # Fourth, cal the content loss
        self.loss_realA_realB = self.SmoothL1Loss(self.real_A[:,:3], self.real_B)* self.opt.lambda_L1
        if torch.is_tensor(self.uv_B):
            self.loss_G_uv_content = self.SmoothL1Loss(self.complete_feat[:,:3][self.index==1], self.uv_B[self.index==1])* self.opt.lambda_L1

        # combine loss and calculate gradients
        # self.loss_G = self.loss_G_GAN + self.loss_G_L1  + self.loss_G_percetual + self.loss_G_face + self.loss_realA_realB
        self.loss_G = self.loss_G_uv_content+self.loss_realA_realB
