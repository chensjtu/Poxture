...
class FaceLoss(nn.Module):
    ...
    def __init__(self, pretrained_path="/media/Diske/projects/CFRH/extradata/assets/checkpoints/losses/sphere20a_20171020.pth", factor=1, device='cuda:0'):
    ...
    def crop_head_bbox(self, imgs, bboxs):
        ...
        for i in range(bs):
            min_x, max_x, min_y, max_y = bboxs[i].int()
    ...
    def find_head_rect(kps, height, width):
        '''
        ----------------------------------------todo----------------------------------------
        todo: change with the smplx mesh.
        '''
        NECK_IDS = 12
...
if __name__=='__main__':
    # test face loss
    crt_face = FaceLoss(factor=1)
    print('net is okay')
    fake_tsf_imgs = torch.randn(2,3,256,256)
    real_tsf_imgs = torch.randn(2,3,256,256)
    mask = torch.tensor([1,0]).float()

    # head_bbox = torch.tensor([[10, 20, 30, 40, 50,60,70,67,88,25,167,10,40, 30, 25,75,90,36,56],
    #     [10, 60, 40, 95, 12,32,45,67,88,25,167,10,20, 92, 25,75,90,36,56]])
    # head_bbox = head_bbox/224.
    head_bbox = torch.tensor([[10, 20, 40, 100],[10, 20, 40, 90]])
    loss_g_face = crt_face(fake_tsf_imgs, real_tsf_imgs,bbox1=head_bbox, bbox2=head_bbox)
    # loss_g_face = crt_face(fake_tsf_imgs, real_tsf_imgs,kps1=head_bbox.T.unsqueeze(0), kps2=head_bbox.T.unsqueeze(0))
    print(loss_g_face)
    # min is better.
    head_bbox2 = torch.tensor([[0, 0, 0, 0],[0, 0, 0, 0]])
    print(crt_face(fake_tsf_imgs, real_tsf_imgs,bbox1=head_bbox2, bbox2=head_bbox2))